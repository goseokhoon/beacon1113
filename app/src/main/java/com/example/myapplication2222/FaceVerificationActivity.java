...
public class FaceVerificationActivity extends AppCompatActivity {
    ...
    private CameraSelector cameraSelector = CameraSelector.DEFAULT_FRONT_CAMERA; // 초기에 전면 카메라로 설정
    private boolean isUsingFrontCamera = true; // 전면 카메라가 사용 중으로 설정
    ...
    private void loadStoredFaceImage() {
        ...
        storageRef.getBytes(Long.MAX_VALUE).addOnSuccessListener(bytes -> {
            ...
            // 사진을 시계방향으로 90도 회전
            storedFaceBitmap = rotateBitmap(originalBitmap, 90);
            ...
            detectFaceUsingMLKit(storedFaceBitmap, new OnFaceDetectedListener() {
                ...
                public void onFaceDetected(Bitmap croppedFace) {
                    if (croppedFace != null) {
                        runOnUiThread(() -> storedFaceImageView.setImageBitmap(croppedFace)); // 회전된 얼굴 이미지 설정
                    } else {
                ...
                public void onFaceDetectionFailed() {
                    ...
                    runOnUiThread(() -> storedFaceImageView.setImageBitmap(storedFaceBitmap)); // 회전된 전체 얼굴을 보여줌
                }
            });
        ...
        imageCapture.takePicture(outputOptions, cameraExecutor, new ImageCapture.OnImageSavedCallback() {
            ...
            public void onImageSaved(@NonNull ImageCapture.OutputFileResults outputFileResults) {
                ...
                if (originalBitmap != null) {
                    // 이미지 회전 없이 원본 비트맵 그대로 사용
                    detectFaceUsingMLKit(originalBitmap, new OnFaceDetectedListener() {
                        @Override
                        public void onFaceDetected(Bitmap croppedFace) {
                            liveFaceBitmap = croppedFace;
                            runOnUiThread(() -> liveFaceImageView.setImageBitmap(croppedFace)); // 원본 얼굴만 UI에 표시
